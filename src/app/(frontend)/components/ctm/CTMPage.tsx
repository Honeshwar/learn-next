...
const LazySubmitFormToSeeResult = dynamic(
  () => import("../SubmitFormToSeeResult"),
  { ssr: false }
);
const LazyResultComponent = dynamic(() => import("./ResultComponent"), {
  ssr: false,
});
// const LazySelectParty = dynamic(() => import("./SelectParty"), {
//   ssr: false,
// });
...
function CTMPage({ lang = "hi" }: { lang: string }) {
  ...
  return (
    <div className="overflow-hidden h-screen bg-blue-900">
      ...
      {screen === 2 && <LazySubmitFormToSeeResult lang={lang} />}

      {/* <!--Result popup  --> */}
      {screen === 3 && <LazyResultComponent lang={lang} />}
    </div>
  );
}
